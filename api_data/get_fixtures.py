...
def get_fixtures_and_update(collection_name, leagues, date=None):
    ...
    try:
        ...
        fixture_ids = database.select_documents_by_where(collection_name, conditions=[
            ('fixture.date', '>=', start_date),
            ('fixture.date', '<', end_date)
        ], receive_ids=True)[1]

        if fixture_ids:
            deleted_fixtures = database.delete_documents_by_ids(collection_name, fixture_ids)
