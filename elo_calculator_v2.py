...
def expected_result(home, away):
    ...
    de = (1 / (math.sqrt(2 * math.pi) * math.e)) * math.exp(-((elo_diff / 200) ** 2) / (2 * math.e ** 2))
    
    weh = (1 / (10 ** (-elo_diff / 400) +1)) - (1/2 * de)
    wea = (1 / (10 ** (elo_diff / 400) +1)) - (1/2 * de)
    
    return [np.round(weh, 3), np.round(wea, 3), np.round(de, 3)]
...
def calculate_elo(elo_home,elo_away,home_goals,away_goals,league):
  ...
  home_expected_result,away_expected_result,de=expected_result(elo_home,elo_away)
  
  elo_home=elo_home+k*g*(home_result-home_expected_result)
  elo_away=elo_away+k*g*(away_result-away_expected_result)
  
  return elo_home,elo_away, home_expected_result, away_expected_result, de